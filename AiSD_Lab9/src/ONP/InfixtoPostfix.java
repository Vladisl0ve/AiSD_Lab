...
public class InfixtoPostfix {
	...
	/*
	 * public String[] SprawdzeniePodwojnych(String arr[]) { String arr1[];
	 * ArrayList<String> arr2 = new ArrayList<String>();
	 * 
	 * for(int i = 0; i < arr.length; i++) { if((isOperator(arr[i]) ==
	 * isOperator(arr[i+1])) && (isOperator(arr[i])== false) ) { arr[i]+=arr[i+1];
	 * arr2.add(arr[i]); i++; continue; } arr2.add(arr[i]);
	 * 
	 * } arr1 = new String[arr2.size()]; arr1 = arr2.toArray(arr1); return arr1; }
	 */
}
